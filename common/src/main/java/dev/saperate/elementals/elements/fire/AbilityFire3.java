...
public class AbilityFire3 implements Ability {
    ...
    public void onTick(Bender bender) {
        ...
        if (!grounded && player.isSprinting() && PlayerData.get(player).canUseUpgrade("fireJet")) {
            float power = 1.35f;
            PlayerData plrData = PlayerData.get(player);
            ...
            serverSummonParticles((ServerLevel) player.level(),
                    PlayerData.get(player).canUseUpgrade("blueFire") ? ParticleTypes.SOUL_FIRE_FLAME : ParticleTypes.FLAME,
                    player, player.getRandom(), 0, 0.1f, 0, 0.1f, 8, 0, -1.5f, 0, 0);
            return;
        }
        ...
        serverSummonParticles((ServerLevel) player.level(),
                PlayerData.get(player).canUseUpgrade("blueFire") ? ParticleTypes.SOUL_FIRE_FLAME : ParticleTypes.FLAME,
                player, player.getRandom(), 0, 0.1f, 0, 0.1f, 1, 0, 0, 0, 0);
    }
