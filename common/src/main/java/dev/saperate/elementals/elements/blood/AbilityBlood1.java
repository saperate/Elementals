...
public class AbilityBlood1 implements Ability {
    ...
    public void onCall(Bender bender, long deltaT) {
        ...
        if (bender.player.isShiftKeyDown() && deltaT >= 1500 && playerData.canUseUpgrade("bloodControl")) {
            BloodElement.get().getAbility(2).onCall(bender, deltaT);
            return;
        } else if (bender.player.isShiftKeyDown() && playerData.canUseUpgrade("bloodShield")) {
