...
public class MetalCableEntityRenderer extends EntityRenderer<MetalCableEntity> {
    ...
    public void render(MetalCableEntity entity, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        ...
        matrices.push();
        Vec3d vec3d = entity.getChild().getLeashPos(tickDelta);
        double d = (double)(MathHelper.lerp(tickDelta, entity.getOwner().prevBodyYaw, entity.getOwner().bodyYaw) * 0.017453292F) + 1.5707963267948966;
        ...
        BlockPos blockPos2 = BlockPos.ofFloored(entity.getChild().getCameraPosVec(tickDelta));
        int q = this.getBlockLight(entity, blockPos);
        int r = entity.getWorld().getLightLevel(LightType.BLOCK, entity.getBlockPos());
    ...
    private static void renderLeashPiece(VertexConsumer vertexConsumer, Matrix4f positionMatrix, float f, float g, float h, int leashedEntityBlockLight, int holdingEntityBlockLight, int leashedEntitySkyLight, int holdingEntitySkyLight, float i, float j, float k, float l, int pieceIndex, boolean isLeashKnot, float distance) {
        ...
        float t = 0.4F * q * q2;
        float u = f * m;
        float v = (float) (g - g * (1.0F - m) * (1.0F - m) - Math.sin(pieceIndex * Math.PI / 24) * Math.min(Math.max(24-distance,0),1));
        float w = h * m;
